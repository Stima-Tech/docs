...
此為 **網頁翻譯擴充程式**。相較以往的 **Google 翻譯** 會直接覆蓋原文，當需要查看原文則必須恢復翻譯，沉浸式翻譯採用 **將譯文插入原文下方**，以進行雙語對照閱讀，當譯文令人困惑時可直接參照原文。此外，沉浸式翻譯 **支援當前熱門的大型語言模型**，如 `GPT-4o`、`Gemini 3 Flash`等模型，顯著提升翻譯精準度。

除翻譯網頁，沉浸式翻譯的官方提供: **翻譯整篇PDF**、**即時翻譯YouTube字幕** 以及 **翻譯 EPUB 電子書** 等功能。以下將詳細介紹如何安裝並使用 **Apertis** 提供之模型。
...
於方框中輸入 `gemini-3-flash-preview, gemini-3-pro-preview, claude-sonnet-4.5`，並儲存。
